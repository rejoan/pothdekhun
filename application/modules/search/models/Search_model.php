...
(r.from_district = ' . $district . $sql_thana . ' AND r.to_district = ' . $to_district . $sql_tothana . ')
OR (r.to_district = ' . $district . $sqlt_thana . ' AND r.from_district = ' . $to_district . $sqlt_tothana . ')', NULL, FALSE);
        if (!$pagination) {
            $this->db->limit($per_page, $segment);
        }
        //$this->db->order_by('r.distance', 'asc');
     ...
     * @return type
     */
    public function get_suggestions($place, $stopage_table, $to_place, $per_page = 10, $segment = 3, $pagination = FALSE, $district, $thana, $to_district, $to_thana) {
        ...
        return $this->final_result($all_routes_id, $per_page, $segment, $pagination, $district, $thana, $to_district, $to_thana);
    }
     ...
     * @return type
     */
    public function final_result($all_routes_id, $per_page, $segment, $pagination, $district, $thana, $to_district, $to_thana, $from_empty,$to_empty) {
        $consider_thana = $this->input->get('c', TRUE);
        //var_dump( $from_empty, $to_empty);return;
        $sql_thana = $sqlt_thana = $sql_tothana = $sqlt_tothana = '';
        if ($district != 1 || $district == $to_district || $consider_thana) {//if not dhaka then filter thana
            $sql_thana .= ' AND r.from_thana = ' . $thana;
            $sqlt_thana .= ' AND r.to_thana = ' . $thana;
        }
