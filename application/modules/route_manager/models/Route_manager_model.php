...
class Route_manager_model extends CI_Model {
    ...
    public function edited_route($route_id) {
        $query = $this->db->select('r.*,p.name,bn_name,added_by poribohon_added,u.*')->from('edited_routes r')->join('poribohons p', 'r.poribohon_id = p.id', 'left')->join('users u', 'u.id = r.added_by', 'left')->where('r.id', $route_id)->get();
        return $query->row_array();
    }
